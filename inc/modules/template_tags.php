			...
			if ( ! $module instanceof Templating_Module_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The theme templating module %1$s does not implement the %2$s interface.', 'noon' ),
						gettype( $module ),
		...
		if ( ! isset( $this->template_tags[ $method ] ) ) {
			throw new BadMethodCallException(
				sprintf(
					...
					__( 'The template tag %s does not exist.', 'noon' ),
					'noon()->' . $method . '()'
				)
			);
		}
		...
		foreach ( $tags as $method_name => $callback ) {
			...
			if ( ! is_array( $callback ) || ! isset( $callback['callback'] ) ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The template tag method %1$s registered by theme module %2$s must either be a callable or an array.', 'noon' ),
						$method_name,
						get_class( $module )
					)
				);
			...
			if ( isset( $this->template_tags[ $method_name ] ) ) {
				throw new RuntimeException(
					sprintf(
						/* translators: 1: template tag method name, 2: module class name */
						__( 'The template tag method %1$s registered by theme module %2$s conflicts with an already registered template tag of the same name.', 'noon' ),
						$method_name,
						get_class( $module )
					)
				);
